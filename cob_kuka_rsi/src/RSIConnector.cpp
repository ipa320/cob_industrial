...
RSIConnector::RSIConnector( std::string own_ip, int udp_port, bool isCorrectingAxes )
{
...
void RSIConnector::parseXML()
{
	...
	try {
		...
		sscanf(_dataFromRobot.c_str(), "<Rob Type=\"KUKA\"><AIPos A1=\"%f\" A2=\"%f\" A3=\"%f\" A4=\"%f\" A5=\"%f\" A6=\"%f\"/><IPOC>%d</IPOC></Rob>",
			&robPos.A1, &robPos.A2, &robPos.A3,
			&robPos.A4, &robPos.A5, &robPos.A6, &robPos.timestamp);
	...
	catch (std::exception e) { std::cerr << "Error while reading xml: " << e.what() << std::endl;getchar(); }
...
void RSIConnector::work()
{
	...
	while (_run)
	{
		try
		{
			...
			{
				boost::thread myParser(boost::bind(&RSIConnector::parseXML, this));
			} 
			_ipoCount++;
